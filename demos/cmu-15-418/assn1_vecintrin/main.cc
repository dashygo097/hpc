...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  __cs149_vec_float x;
  __cs149_vec_int y;
  __cs149_mask maskAll, maskIsNotZero, maskIsOverflow, maskIsNotOverflow;
  __cs149_vec_int zero_int = _cs149_vset_int(0), one_int = _cs149_vset_int(1);
  __cs149_vec_float one_float = _cs149_vset_float(1.f),
                    overfloaw_float = _cs149_vset_float(9.999999f);

  maskAll = _cs149_init_ones();

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    __cs149_vec_float result = one_float;
    _cs149_vload_float(x, values + i, maskAll);             // x = values[i];
    _cs149_vload_int(y, exponents + i, maskAll);            // y = exponents[i];
    _cs149_vgt_int(maskIsNotZero, y, zero_int, maskAll);    // if (y == 0) {
    while (_cs149_cntbits(maskIsNotZero)) {                 // while (y > 0) {
      _cs149_vmult_float(result, result, x, maskIsNotZero); // result *= x;
      _cs149_vsub_int(y, y, one_int, maskIsNotZero);        // y--;
      _cs149_vgt_int(maskIsNotZero, y, zero_int, maskIsNotZero);
    }
    _cs149_vgt_float(maskIsOverflow, result, overfloaw_float,
...
float arraySumVector(float *values, int N) {
  ...
  __cs149_vec_float x = _cs149_vset_float(0.0f),
                    result = _cs149_vset_float(0.0f);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    _cs149_vload_float(x, values + i, maskAll);
    _cs149_vadd_float(result, result, x, maskAll); // result += x;
  }

  _cs149_vstore_float(sum_vec, result, maskAll);
