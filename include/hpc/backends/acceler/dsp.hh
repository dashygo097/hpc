...
template <> struct dsp_traits<float> {
  ...
  vsadd(const float *src, float scalar, float *dst, size_t n) {
    vDSP_vsadd(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsmul(const float *src, float scalar, float *dst, size_t n) {
    vDSP_vsmul(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsdiv(const float *src, float scalar, float *dst, size_t n) {
    vDSP_vsdiv(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vadd(const float *a, const float *b, float *dst, size_t n) {
    vDSP_vadd(a, 1, b, 1, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsub(const float *a, const float *b, float *dst, size_t n) {
    vDSP_vsub(b, 1, a, 1, dst, 1, n); // dst = a - b
  }

  __attribute__((always_inline)) static inline void
  vmul(const float *a, const float *b, float *dst, size_t n) {
    vDSP_vmul(a, 1, b, 1, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vdiv(const float *a, const float *b, float *dst, size_t n) {
...
template <> struct dsp_traits<double> {
  ...
  __attribute__((always_inline)) static inline void
  vsadd(const double *src, double scalar, double *dst, size_t n) {
    vDSP_vsaddD(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsmul(const double *src, double scalar, double *dst, size_t n) {
    vDSP_vsmulD(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsdiv(const double *src, double scalar, double *dst, size_t n) {
    vDSP_vsdivD(src, 1, &scalar, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vadd(const double *a, const double *b, double *dst, size_t n) {
    vDSP_vaddD(a, 1, b, 1, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vsub(const double *a, const double *b, double *dst, size_t n) {
    vDSP_vsubD(b, 1, a, 1, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  vmul(const double *a, const double *b, double *dst, size_t n) {
  ...
  __attribute__((always_inline)) static inline void
  vfill(double value, double *dst, size_t n) {
    vDSP_vfillD(&value, dst, 1, n);
  }

  __attribute__((always_inline)) static inline void
  axpy(size_t n, double alpha, const double *x, double *y) {
