...
inline void gemm_simd(const size_t &M, const size_t &K, const size_t &N,
        ...
        for (size_t i = 0; i < TileSize; ++i) {
          ...
          for (size_t k = 0; k < TileSize; ++k) {
            ...
            for (; j + SimdWidth <= TileSize; j += SimdWidth) {
              const simd_t b_vec = SIMD_LOAD(traits, b_row + j);
              const simd_t c_vec = SIMD_LOAD(traits, c_row + j);
