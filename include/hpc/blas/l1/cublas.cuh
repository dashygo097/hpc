...
#if defined(HPC_ENABLE_CUDA) && defined(ENABLE_CUBLAS) && defined(__CUDACC__)
#include "../../backends/backends.hh"
#endif

#if defined(HPC_ENABLE_CUDA) && defined(ENABLE_CUBLAS) && defined(__CUDACC__)
#define HPC_ENABLE_CUBLAS_VECTOR_SCALAR_BRANCH(name)                           \
  else if constexpr (backend == Backend::CUBLAS) {                             \
    details::name##_cublas<T, BackendParams...>(n, dst, src, scalar);          \
  }
#define HPC_ENABLE_CUBLAS_BINARY_BRANCH(name)                                  \
  else if constexpr (backend == Backend::CUBLAS) {                             \
    details::name##_cublas<T, BackendParams...>(n, dst, src1, src2);           \
  }
#define HPC_ENABLE_CUBLAS_UNARY_BRANCH(name)                                   \
  else if constexpr (backend == Backend::CUBLAS) {                             \
    details::name##_cublas<T, BackendParams...>(n, dst, src);                  \
  }
#define HPC_ENABLE_CUBLAS_SCALAR_BRANCH(name)                                  \
  else if constexpr (backend == Backend::CUBLAS) {                             \
    details::name##_cublas<T, BackendParams...>(n, dst, scalar);               \
  }
#define HPC_ENABLE_CUBLAS_REDUCE_BRANCH(name)                                  \
  else if constexpr (backend == Backend::CUBLAS) {                             \
    return details::name##_cublas<T, BackendParams...>(n, src);                \
  }
#define HPC_ENABLE_CUBLAS_REDUCE2_BRANCH(name)                                 \
  else if constexpr (backend == Backend::CUBLAS) {                             \
